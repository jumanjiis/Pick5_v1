...
import { AlertCircle, Save, Trophy, X, Star } from 'lucide-react';
...
const MatchPrediction = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (new Date() >= matchData.timestamp) {
        ...
        if (!predictionsSnapshot.empty) {
          ...
          setSelectedPlayers(playersWithTargets.filter(p => selectedPlayerIds.includes(p.id)));
  ...
  const handleSubmit = async () => {
    if (!match || selectedPlayers.length !== 5 || !user || submitting) return;
    setSubmitting(true);
                  ...
                  {player ? (
                    <>
                      ...
                      <div className={`flex flex-col items-center text-center ${
                        ...
                        <div className="mt-2 px-3 py-1 rounded-full bg-green-500/20 text-green-200 text-sm">
